 ...
 * @author Alvarez
 */
public class lEgresos extends javax.swing.JPanel {
     ...
     */
    public lEgresos(EntityManagerFactory factory) {
        initComponents();
    ...
    private void setDatos() {
        ...
        for (VariedadesEgresos vi : vijc.findVariedadesEgresosEntities()) {
            if (vi.getEstado() != 0 && vi.getFechaegreso().before(new Date(jDateChooser2.getDate().getYear(), jDateChooser2.getDate().getMonth(), jDateChooser2.getDate().getDate(), 0, 0, 0)) && vi.getFechaegreso().after(new Date(jDateChooser1.getDate().getYear(), jDateChooser1.getDate().getMonth(), jDateChooser1.getDate().getDate(), 23, 59, 59))) {
                model.addRow(dato);
                model.setValueAt(vi, model.getRowCount() - 1, 0);
                model.setValueAt(vi.getCodigoegreso(), model.getRowCount() - 1, 1);
                ...
                model.setValueAt(Funciones.ddMMyyyy.format(vi.getFechaegreso()), model.getRowCount() - 1, 3);
                model.setValueAt(vi.getFechaegresotiempo(), model.getRowCount() - 1, 4);
                model.setValueAt(vi.getEgresoItemList().get(0).getDescripcion(), model.getRowCount() - 1, 5);
    ...
    private void setDatosSuministro() {
        DecimalFormat formato = new DecimalFormat("#,###.00");
        for (VariedadesSuministro vi : vsjc.findVariedadesSuministroEntities()) {
            if (vi.getEstado() != 0 && vi.getFechaventa().before(new Date(jDateChooser2.getDate().getYear(), jDateChooser2.getDate().getMonth(), jDateChooser2.getDate().getDate(), 0, 0, 0)) && vi.getFechaventa().after(new Date(jDateChooser1.getDate().getYear(), jDateChooser1.getDate().getMonth(), jDateChooser1.getDate().getDate(), 23, 59, 59))) {
                model.addRow(dato);
                model.setValueAt(vi, model.getRowCount() - 1, 0);
                model.setValueAt(vi.getCodigosuministro(), model.getRowCount() - 1, 1);
                if (obtenerUser(vi.getCodcliente()).size() > 0) {
                    usuarios = obtenerUser(vi.getCodcliente()).get(0);
                    model.setValueAt(usuarios.getNombres() + " " + usuarios.getApellidos(), model.getRowCount() - 1, 2);
                } else if (obtenerTercero(vi.getCodcliente()).size() > 0) {
                ...
                model.setValueAt("SUMINISTRO", model.getRowCount() - 1, 5);
                total = total + vi.getValortotal();
                model.setValueAt(formato.format(vi.getValortotal()), model.getRowCount() - 1, 6);
                model.setValueAt(vi.getIdusuario(), model.getRowCount() - 1, 7);
                model.setValueAt(vi.getEstado(), model.getRowCount() - 1, 8);
    ...
    private void exportarToExcel() {
        try {
            if (jTable1.getRowCount() > 0) {
                ...
                File file = new File(ruta + "InformeEgresosdel" + Funciones.ddMMyyyy2.format(jDateChooser1.getDate()) + "al" + Funciones.ddMMyyyy2.format(jDateChooser2.getDate()) + ".xls");
                crearExcel(file);
            } else {
    ...
    private void crearExcel(File file) {
        ...
        try {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                if (!model.getValueAt(i, 5).toString().equals("SUMINISTRO")) {
                    ...
                    excelSheet.addCell(new Label(1, count, model.getValueAt(i, 2).toString(), cf));//CLIENTE
                    excelSheet.addCell(new Label(2, count, model.getValueAt(i, 3).toString(), cf));//FECHA   
                    excelSheet.addCell(new Label(3, count, "SUMINISTRO", cf));//DESCRIPCION  
                    excelSheet.addCell(new Label(4, count, model.getValueAt(i, 6).toString(), cf));//TOTAL                  
                }
                count++;
            }
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            ...
                            .addComponent(bExportar, javax.swing.GroupLayout.DEFAULT_SIZE, 31, Short.MAX_VALUE)))
    ...
    private void jLabel2MouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel2MouseReleased
        String mensaje = "¿Desea adjuntar los suministros al reporte de egresos?";
        int entrada = JOptionPane.showConfirmDialog(null, mensaje, "Adjuntar suministros", JOptionPane.YES_NO_OPTION);
        if (entrada == 0) {
