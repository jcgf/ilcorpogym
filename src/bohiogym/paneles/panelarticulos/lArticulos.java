 ...
 * @author Alvarez
 */
public class lArticulos extends javax.swing.JPanel {
    ...
    /**
     * Creates new form lArticulos
     */
    public lArticulos(EntityManagerFactory factory) {
        initComponents();
    ...
    private DefaultTableModel getModelo() {
        DefaultTableModel modelo = new DefaultTableModel(
                null, new String[]{"id", "Codigo", "Descripcion", "Precio", "Can."}) {
    ...
    private void setCargarTabla() {
        ...
        jTable1.getColumnModel().getColumn(2).setMinWidth(260);
        jTable1.getTableHeader().getColumnModel().getColumn(2).setMaxWidth(260);
    ...
    private void setDatos() {
        ...
        for (VariedadesArticulos va : obtenerArticulo()) {
            ...
            model.setValueAt("<html><div style=\"width:260;\">" + va.getNombrearticulo() + "</div></html>", model.getRowCount() - 1, 2);
            for (int i = 0; i < art.size(); i++) {
                if (va.toString().equals(art.get(i).getIdarticulo().toString())) {
                    canti = canti + art.get(i).getCantidad();
                }
            }
            for (int h = 0; h < das.size(); h++) {
                if (va.toString().equals(das.get(h).getIdarticulo().toString())) {
                    canti = canti + das.get(h).getCantidad();
                }
            }
            for (int k = 0; k < sas.size(); k++) {
                if (va.toString().equals(sas.get(k).getIdarticulo().toString())) {
                    canti = canti + sas.get(k).getCantidad();
                }
            }
            for (int j = 0; j < vart.size(); j++) {
                if (va.toString().equals(vart.get(j).getIdarticulo().toString())) {
                    canti = canti - vart.get(j).getCantidad();
                }
            }
            model.setValueAt(va.getValorventa(), model.getRowCount() - 1, 3);
    ...
    private void initComponents() {
        ...
        tFiltro.setText("Ingrese código o nombre del articulo");
